...
infile = HOMEDIR+'sidm_orbit_calculation/src/output/%i_%s_%s_%i.dat' %(host_idx,integrator,potential,sub_idx)
...
mt_x = mt_x[mt_dist < dist.max()]
mt_y = mt_y[mt_dist < dist.max()]
mt_z = mt_z[mt_dist < dist.max()]
...
plt.plot(0,0,'^k')
plt.plot(x[0],y[0],'c*',markersize=10)
