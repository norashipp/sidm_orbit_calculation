...
for sub_idx in sub_idx_array:
        if host.subhalos[sub_idx]:
            ...
            if triaxial:
                    ...
                    infile_triaxial = HOMEDIR+'scratch-midway/%i_%s_%s_%.0e_%i.dat' %(host_idx,integrator,potential,dt,sub_idx)
            ...
            plt.figure()
            plt.plot(t,dist/dmt_sp(t),lw=3,color='g')
            if triaxial: plt.plot(t,dist_tri/dmt_sp(t),lw=3,color='r')
            plt.xlabel(r'$\mathrm{Gyr}$')
            plt.ylabel(r'$\mathrm{Distance\ Ratio}$')
            plt.savefig(HOMEDIR + '/sidm_orbit_calculation/src/plots/%i_%.0e_%i_ratio.png'%(host_idx,dt,sub_idx))
